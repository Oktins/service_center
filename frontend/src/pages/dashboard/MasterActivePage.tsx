...
import { RequestStatus, PRIORITY_LABELS, type SparePart } from '../../types';
...
interface AddPartModalProps {
  spareParts: { content: SparePart[] } | undefined;
  onClose: () => void;
  onUse: (partId: number, qty: number) => void;
  isPending: boolean;
}

const AddPartModal = ({ spareParts, onClose, onUse, isPending }: AddPartModalProps) => {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6 animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={isPending} className="btn-primary w-full mt-4">
            {isPending ? 'Добавление...' : 'Списать со склада'}
          </button>
        </form>
      </div>
    </div>
    ...
    <div>
      ...
      {selectedRequest && (
        <AddPartModal 
          ...
          onUse={(partId, qty) => usageMutation.mutate({ reqId: selectedRequest, partId, qty })}
